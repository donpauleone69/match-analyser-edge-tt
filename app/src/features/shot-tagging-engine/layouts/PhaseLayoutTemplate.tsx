 ...
 * 
 * MOBILE (< 768px):
 * Vertical stack: Shot Log | Video | Status | Input
 * 
 * DESKTOP (≥ 768px):
...
export interface PhaseLayoutTemplateProps {
  ...
  userInput: ReactNode
  className?: string
  shotLogRef?: React.RefObject<HTMLDivElement | null>
}
...
export function PhaseLayoutTemplate({
  ...
  return (
    <div className={cn('fixed inset-0 flex flex-col md:grid md:grid-cols-[320px_1fr] md:grid-rows-[1fr_auto_auto] bg-bg-surface overflow-hidden', className)}>
      {/* Shot Log - Left sidebar on desktop, top on mobile */}
      <div 
        ref={shotLogRef}
        className="flex-1 min-h-0 overflow-y-auto p-4 space-y-2 bg-bg-surface md:row-span-3 md:border-r md:border-neutral-700"
      >
        {shotLog}
      </div>
